  ...
  <div class="recentLooking text-left mt15" v-if="successBack" v-loading="recentBookLoading">
    <div class="recent1">
      ...
      <div class="inline-block font-white recent1-1 ml30 mt20 hvr-float-shadow">
        <img style="vertical-align:middle" class="recent-book-img" @click="lookArticle('recent', true)" :src="Recent.picBase64" v-if="this.Recent.picBase64 !== ''" alt=""/>
        <img style="vertical-align:middle" class="recent-book-img" @click="lookArticle('recent', true)" src="../../assets/Image/图片无法加载1.jpg" v-else alt=""/>
      </div>
      ...
      <div class="inline-block font-white recent1-2 ml60 mt20 hvr-float-shadow" v-for="i of Recommend">
        <img style="vertical-align:middle" class="recent-book-img" @click="lookArticle(i, false)" :src="i.picBase64" v-if="i.picBase64 !== ''" alt=""/>
        <img style="vertical-align:middle" class="recent-book-img" @click="lookArticle(i, false)" src="../../assets/Image/图片无法加载1.jpg" v-else="i.picBase64 === ''" alt=""/>
      </div>
    </div>
    <div class="recent2">
      <div class="recent2-1 text-center float-l">
        <span class="inline-block ml40 font18 font-white hvr-wobble-top">{{Recent.title}}</span>
      </div>
...
export default {
  ...
  methods: {
    lookArticle (item, bool) {
      ...
      var booktitle
      if (bool) {
        bookusername = this.Recent.username
        booktitle = this.Recent.title
      } else {
